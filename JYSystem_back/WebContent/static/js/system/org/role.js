$(function () {
	...
	$("#authorityDiv button").bind('click', function(){		
		...
		var thelayer=$(this).val();
		$("#authorityDiv input[name$='layer']").val(thelayer);
		//重新加载资源
		var auth=$("#authorityDiv input[name$='auth']").val();
		if(auth=='org'){
			var orgId=$("#baseForm input[name$='orgId']").val();
			JY.Ajax.doRequest("",jypath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:thelayer},function(data){
			...
			JY.Ajax.doRequest(null,jypath +'/backstage/org/role/listAuthorized',{id:roleId,layer:thelayer},function(data){
				$.fn.zTree.init($("#authorityTree"),{check: {enable: true,chkDisabledInherit: true,chkboxType:{"Y":"ps","N":"s"}},data:{simpleData:{enable: true}}},data.obj);	      
			});
		}else{
	...
	$('#authOrgBtn').on('click', function(e) {
		...
		e.preventDefault();
		var orgId=$("#baseForm input[name$='orgId']").val();
		var thelayer=$("#authorityDiv input[name$='layer']").val();
			...
			JY.Ajax.doRequest("",jypath +'/backstage/org/role/orglistAuthorized',{id:orgId,layer:thelayer},function(data){
			         ...
			         JY.Model.edit("authorityDiv","机构组织授权",function(){
			        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
		 					...
		 					var that =$(this);
		 					thelayer=$("#authorityDiv input[name$='layer']").val();
		 					JY.Ajax.doRequest(null,jypath +'/backstage/org/role/saveOrgAuthorized',{id:orgId,aus:aus,layer:thelayer},function(data){
		 	 					 that.dialog("close");
...
function authorized(id){
	...
	var thelayer=$("#authorityDiv input[name$='layer']").val();
	JY.Ajax.doRequest(null,jypath +'/backstage/org/role/listAuthorized',{id:id,layer:thelayer},function(data){
	         //获取数据
	    	 var zNodes=data.obj;
	         //设置
	         ...
	         JY.Model.edit("authorityDiv","角色授权",function(){
	        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
 					...
 					var that =$(this);
 					thelayer=$("#authorityDiv input[name$='layer']").val();
 					JY.Ajax.doRequest(null,jypath +'/backstage/org/role/saveAuthorized',{id:id,aus:aus,layer:thelayer},function(data){
 	 					  that.dialog("close");
 	 					  layer.msg(data.resMsg);search();			        		 	
 					});
	         });
